...
import com.example.adoptasv.VoluntarioActivity;
...
/**
 * Inicio del módulo del voluntario: stats de su refugio + accesos rápidos a las
 * tres áreas de gestión (mascotas, solicitudes, reportes). Misma estructura visual
 * que el dashboard admin pero sin las acciones exclusivas de administración.
 */
public class VoluntarioDashboardFragment extends Fragment {

    private TextView tvStatDisponibles, tvStatPendientes, tvStatSeguimientos, tvSubtitulo;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_voluntario_dashboard, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        ...
        view.findViewById(R.id.cardMascotas).setOnClickListener(v -> irATab(R.id.nav_mascotas));
        view.findViewById(R.id.cardSolicitudes).setOnClickListener(v -> irATab(R.id.nav_solicitudes));
        view.findViewById(R.id.cardReportes).setOnClickListener(v -> irATab(R.id.nav_reportes));

        cargarResumen();
        cargarRefugio();
    }

    private void irATab(int menuId) {
        if (getActivity() instanceof VoluntarioActivity)
            ((VoluntarioActivity) getActivity()).setTab(menuId);
    }
    ...
    private void cargarRefugio() {
        ...
        // getPerfil incluye el refugio del usuario (auth/me no lo trae).
        ApiClient.getService().getPerfil().enqueue(new Callback<User>() {
            ...
            public void onResponse(@NonNull Call<User> call, @NonNull Response<User> response) {
                ...
                if (response.isSuccessful() && u != null
                        && u.refugio != null && u.refugio.nombre != null) {
